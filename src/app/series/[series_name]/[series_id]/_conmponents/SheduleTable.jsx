...
export default function SheduleTable({ data }) {
  return (
    <div className="">
      <div className="mx-auto bg-white overflow-hidden">
        <table className="w-full text-left">
          ...
          <tbody className="text-gray-600">
            {data?.map((item, index) => {
              if (!item?.matchDetailsMap) return null; // Skip if no matchDetailsMap

              const matchInfo = item?.matchDetailsMap?.match[0]?.matchInfo;
