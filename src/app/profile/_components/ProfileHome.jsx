...
export default function ProfileHome() {
  ...
  const handleCancelPayment = async () => {
    ...
    try {
      ...
      if (response.status === 200) {
        toast.success('Subscription cancelled successfully.');
        refetch();
        updateUser({ ...user, isUnsubscribed: true });

        // Note: If you logout and push immediately, the user won't see the "Inactive" state.
        // You might want to delay this or remove logout if they should see the 'Enjoy until' message.
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 mt-10 border border-gray-300 rounded-lg p-4 lg:p-6">
      {/* Profile Section */}
      <div className=" bg-white shadow-md rounded-xl p-6 flex flex-col items-start gap-4">
        <div className="relative w-28 h-28">
          <img
            src="/images/default_profile.png"
            alt="Profile"
          ...
          />
          <span
            className={`absolute bottom-1 right-1 w-5 h-5 border-2 border-white rounded-full ${user?.isUnsubscribed ? 'bg-gray-400' : 'bg-green-500'}`}
          ></span>
        </div>
        ...
        <div className="w-full text-start space-y-2">
          ...
          {/* Requirement: Next Auto-Renewal Date as NA after deactivation */}
          <p className="text-lg font-medium text-gray-800">
            <span className="font-semibold">Next Auto-Renewal Date:</span>{' '}
            {!user?.isUnsubscribed && user?.expiresAt
              ? formatted(user.expiresAt * 1000)
              : 'N/A'}
          </p>
          {/* Requirement: Display deactivation status */}
          {user?.isUnsubscribed && (
        ...
        {/* Requirement: Disable button after unsubscribe */}
        <button
          className={`w-full mt-4 py-2 rounded-lg font-semibold transition-colors duration-200 
            ${
              isProcessing || user?.isUnsubscribed
                ? 'bg-gray-300 text-gray-600 cursor-not-allowed'
                : 'bg-red-500 text-white hover:bg-red-600'
            }
          `}
          onClick={handleUnsubscribeClick}
          disabled={isProcessing || user?.isUnsubscribed}
        >
          {isProcessing
            ? 'Processing...'
            : user?.isUnsubscribed
      ...
      <div className="p-6 max-w-md w-full mx-auto bg-white shadow-md rounded-xl">
        ...
        {user?.reference || user?.isUnsubscribed ? (
          <div
            className={`border-2 rounded-lg p-4 mb-4 transition-all duration-200 ${user?.isUnsubscribed ? 'border-gray-300 bg-gray-50' : 'border-red-500 bg-red-50'}`}
          >
            <label className="flex items-center justify-between cursor-pointer">
              <div className="flex items-center gap-3">
                <input
                  ...
                  name="plan"
                  checked
                  className={`form-radio checkbox-md ${user?.isUnsubscribed ? 'checkbox-ghost' : 'checkbox-error'}`}
                  readOnly
                />
                <span
                  className={`text-lg font-semibold ${user?.isUnsubscribed ? 'text-gray-500' : 'text-red-600'}`}
                >
                  {user?.membershipPlan}
                </span>
              </div>
              {/* Requirement: Show Inactive and Enjoy until text */}
              <span
                className={`text-sm font-medium ${user?.isUnsubscribed ? 'text-gray-500' : 'text-red-700'}`}
              >
                {user?.isUnsubscribed ? 'Inactive' : 'Active'}
              </span>
            </label>
            {user?.isUnsubscribed && user?.expiresAt && (
              <p className="text-xs text-gray-500 mt-2 italic">
                Enjoy the service until {formatted(user.expiresAt * 1000)}
              </p>
            )}
          </div>
        ) : isLoading ? (
